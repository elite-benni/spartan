...
export class BrnDialogComponent {
	...
	public readonly state = input<BrnDialogState | null>(null, { alias: 'state' });
	...
	public readonly roleState = computed(() => this.mutableRole()());
	private readonly _roleEffect = effect(() => {
		this._options.role = this.roleState();
	});

	public readonly hasBackdrop = input(true, { transform: booleanAttribute });
	public readonly mutableHasBackdrop = computed(() => signal(this.hasBackdrop()));
	public readonly hasBackdropState = computed(() => this.mutableHasBackdrop()());
	private readonly _hasBackdropEffect = effect(() => {
		this._options.hasBackdrop = this.hasBackdropState();
	});

	public readonly positionStrategyInput = input<BrnDialogOptions['positionStrategy']>(null, {
		alias: 'positionStrategy',
	});
	public readonly mutablePositionStrategy = computed(() => signal(this.positionStrategyInput()));
	public readonly positionStrategy = computed(() => this.mutablePositionStrategy()());
	...
	public readonly mutableCloseOnOutsidePointerEvents = computed(() => signal(this.closeOnOutsidePointerEventsInput()));
	public readonly closeOnOutsidePointerEvents = computed(() => this.mutableCloseOnOutsidePointerEvents()());
	...
	public readonly mutableAttachTo = computed(() => signal(this.attachToInput()));
	public readonly attachTo = computed(() => this.mutableAttachTo()());
	...
	public readonly attachPositionsState = computed(() => this.mutableAttachPositions()());
	private readonly _attachPositionsEffect = effect(() => {
		this._options.attachPositions = this.attachPositionsState();
	});
